...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options["clear"]:
            ...
            PeriodicTask.objects.filter(name__startswith="Stock Data:").delete()
        ...
        # Create schedules
        _hourly_schedule, created = IntervalSchedule.objects.get_or_create(
            every=1,
        ...
        _market_close_schedule, created = CrontabSchedule.objects.get_or_create(
            minute=0,
            hour=16,
            day_of_week="1,2,3,4,5",  # Monday to Friday
            day_of_month="*",
        ...
        _midday_schedule, created = CrontabSchedule.objects.get_or_create(
            minute=0,
            hour=12,
            day_of_week="1,2,3,4,5",  # Monday to Friday
            day_of_month="*",
        ...
        for task_config in tasks:
            _task, created = PeriodicTask.objects.get_or_create(
                ...
                defaults={
                    ...
                    "crontab": task_config.get("schedule")
        ...
        self.stdout.write(
            f"Total active tasks: {PeriodicTask.objects.filter(enabled=True, name__startswith='Stock Data:').count()}"
        )
