...
from stocks.models import Stock, UserWatchlist
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        stocks_to_delete = stocks_with_counts.filter(price_count=0, intraday_count=0)
        ...
        if min_days > 0:
            ...
            stocks_with_sufficient_data = (
                Stock.objects.filter(
                    Q(prices__date__gte=cutoff_date)
                    | Q(intraday_prices__timestamp__date__gte=cutoff_date)
                )
                .annotate(
                    recent_price_days=Count(
                        "prices__date",
                        distinct=True,
                        filter=Q(prices__date__gte=cutoff_date),
                    ),
                    recent_intraday_days=Count(
                        "intraday_prices__timestamp__date",
                        distinct=True,
                        filter=Q(intraday_prices__timestamp__date__gte=cutoff_date),
                    ),
                )
                .filter(
                    Q(recent_price_days__gte=min_days)
                    | Q(recent_intraday_days__gte=min_days)
                )
                .distinct()
            )
        ...
        if exclude_watchlist:
            watchlist_stocks = UserWatchlist.objects.values_list(
                "stock_id", flat=True
            ).distinct()
            stocks_to_delete = stocks_to_delete.exclude(id__in=watchlist_stocks)
            self.stdout.write(
                f"Excluding {watchlist_stocks.count()} stocks that are in watchlists"
            )
        ...
        if not dry_run:
            try:
                with transaction.atomic():
                    ...
                    for stock in stocks_to_delete_list:
                        ...
                        if deleted_count % 100 == 0:
                            self.stdout.write(
                                f"Deleted {deleted_count}/{count_to_delete} stocks..."
                            )
                ...
                self.stdout.write(self.style.ERROR(f"\n✗ Error deleting stocks: {e!s}"))
                msg = f"Failed to delete stocks: {e!s}"
