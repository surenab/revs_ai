...
from stocks.models import IntradayPrice, Stock
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options["force"]:
            ...
            stocks_with_data = (
                IntradayPrice.objects.filter(
                    timestamp__date=today, interval=options["interval"]
                )
        ...
        if not stocks.exists():
            self.stdout.write(
                self.style.SUCCESS("All stocks already have today's intraday data")
            )
            return
        ...
        for i in range(0, len(stock_list), batch_size):
            ...
            if options["dry_run"]:
                self.stdout.write(
                    f"[DRY RUN] Would fetch {interval} data for: {', '.join(batch_symbols)}"
                )
            ...
            try:
                ...
                for stock in batch:
                    try:
                        data = yahoo_finance_service.get_intraday_data(
                            symbol=stock.symbol, interval=interval, period=period
                        )
                        ...
                        if data and data.get("data"):
                            ...
                            success_count += 1
                            self.stdout.write(
                                f"✓ {stock.symbol}: {len(data['data'])} data points"
                            )
                        else:
                            error_count += 1
                            self.stdout.write(
                                self.style.WARNING(
                                    f"✗ {stock.symbol}: No data received"
                                )
                            )
                ...
                self.stdout.write(self.style.ERROR(f"✗ Batch error: {e!s}"))
        ...
        try:
            with transaction.atomic():
                ...
                for point in data["data"]:
                    # Parse datetime
                    dt = datetime.strptime(point["datetime"], "%Y-%m-%d %H:%M:%S")  # noqa: DTZ007
