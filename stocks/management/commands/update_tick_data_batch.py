...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        today_start = eastern.localize(datetime.combine(now_eastern.date(), dt_time.min))
        today_end = eastern.localize(datetime.combine(now_eastern.date(), dt_time.max))
        ...
        if skip_existing:
            stocks_with_ticks = StockTick.objects.filter(
            ...
            if skipped_count > 0:
                self.stdout.write(
                    self.style.SUCCESS(f'Skipping {skipped_count} stocks with existing tick data for today')
                )
        ...
        self.stdout.write(f'Processing {len(stocks_list)} stocks in {total_batches} batches...\n')
        ...
        for batch_num in range(0, len(stocks_list), batch_size):
            ...
            try:
                ...
                quotes_data = yahoo_finance_service.get_multiple_current_quotes(batch_symbols)
                ...
                for stock in batch_stocks:
                    ...
                    try:
                        ...
                        if 'timestamp' in quote_data:
                            try:
                                tick_timestamp = datetime.strptime(
                        ...
                        if tick_timestamp.date() != now_eastern.date():
                            batch_failed += 1
                            failed_symbols[symbol] = f"Timestamp not for today: {tick_timestamp.date()}"
                            continue

                        # Determine if market hours
                        tick_is_market_hours = is_market_open() if not force_market_hours else True
                        ...
                        tick = StockTick(
                            ...
                            bid_price=quote_data.get('bid') if quote_data.get('bid') else None,
                            ask_price=quote_data.get('ask') if quote_data.get('ask') else None,
                            bid_size=quote_data.get('bid_size') if quote_data.get('bid_size') else None,
                            ask_size=quote_data.get('ask_size') if quote_data.get('ask_size') else None,
                        ...
                        logger.error(f"Error creating tick for {symbol}: {e}")
                        batch_failed += 1
                        failed_symbols[symbol] = f"Data validation error: {str(e)}"
                    except Exception as e:
                        logger.error(f"Error creating tick for {symbol}: {e}")
                        batch_failed += 1
                        failed_symbols[symbol] = f"Unexpected error: {str(e)}"
                ...
                if ticks_to_create:
                    try:
                        ...
                        logger.error(f"Error bulk creating ticks for batch: {e}")
                        self.stdout.write(
        ...
        self.stdout.write(
            self.style.ERROR(f'Failed updates: {failed_updates}')
        )
        self.stdout.write(
        ...
        if failed_symbols:
            ...
            for error_type, symbols in error_groups.items():
                ...
                if len(symbols) <= 10:
                    ...
                    self.stdout.write(f'    {", ".join(symbols[:10])}... and {len(symbols) - 10} more')

        # Show tick data statistics for today
        today_ticks_count = StockTick.objects.filter(
            timestamp__date=now_eastern.date()
        ).count()
        unique_stocks_with_ticks = StockTick.objects.filter(
            timestamp__date=now_eastern.date()
