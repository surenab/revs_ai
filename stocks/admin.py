...
class StockAdmin(admin.ModelAdmin):
    list_display = [
        'symbol', 'name', 'exchange', 'sector', 'market_cap_display',
...
class StockPriceAdmin(admin.ModelAdmin):
    list_display = [
        'stock_symbol', 'date', 'interval', 'open_price', 'close_price',
        'price_change_display', 'volume_display', 'created_at'
    ]
    list_filter = ['interval', 'date', 'stock__exchange', 'created_at']
    search_fields = ['stock__symbol', 'stock__name']
    readonly_fields = ['id', 'price_change', 'price_change_percent', 'created_at', 'updated_at']
    date_hierarchy = 'date'
    ordering = ['-date', 'stock__symbol']

    fieldsets = (
        ('Stock Information', {
            'fields': ('stock', 'date', 'interval')
        }),
        ('Price Data', {
            'fields': ('open_price', 'high_price', 'low_price', 'close_price', 'adjusted_close')
        }),
        ('Volume', {
            'fields': ('volume',)
        }),
...
class UserWatchlistAdmin(admin.ModelAdmin):
    list_display = [
        'user_email', 'stock_symbol', 'target_price', 'created_at'
    ]
...
class StockAlertAdmin(admin.ModelAdmin):
    list_display = [
        'user_email', 'stock_symbol', 'alert_type', 'threshold_value',
        'is_active', 'is_triggered', 'created_at'
    ]
    list_filter = [
        'alert_type', 'is_active', 'is_triggered', 'created_at', 'stock__exchange'
    ]
    search_fields = ['user__email', 'stock__symbol', 'stock__name']
...
class StockTickAdmin(admin.ModelAdmin):
    list_display = [
        'stock_symbol', 'price', 'volume', 'bid_price', 'ask_price',
        'spread_display', 'trade_type', 'timestamp', 'is_market_hours'
    ]
    ...
    fieldsets = (
        ...
        ('Calculated Fields', {
            'fields': ('spread', 'spread_percentage'),
            'classes': ('collapse',)
        }),
        ('Metadata', {
...
class IntradayPriceAdmin(admin.ModelAdmin):
    list_display = [
        'stock_symbol', 'timestamp', 'interval', 'session_type',
        'open_price', 'close_price', 'price_change_display',
        'volume_display', 'vwap', 'trade_count'
    ]
    list_filter = ['interval', 'session_type', 'timestamp', 'stock__exchange']
    search_fields = ['stock__symbol', 'stock__name']
    readonly_fields = ['id', 'price_change', 'price_change_percent', 'typical_price', 'created_at']
    date_hierarchy = 'timestamp'
    ordering = ['-timestamp']
    ...
    fieldsets = (
        ...
        ('Volume and Metrics', {
            'fields': ('volume', 'vwap', 'trade_count')
        }),
        ('Calculated Fields', {
    ...
    def price_change_display(self, obj):
        ...
        if change > 0:
            format_html(f'<span style="color: red;">${change} ({change_percent:.4f}%)</span>')
        elif change < 0:
            return format_html(f'<span style="color: red;">${change} ({change_percent:.4f}%)</span>')
...
class PortfolioAdmin(admin.ModelAdmin):
    list_display = [
        'user_email', 'stock_symbol', 'quantity', 'purchase_price',
        'purchase_date', 'total_cost_display', 'current_value_display',
        'gain_loss_display', 'created_at'
    ]
    list_filter = ['purchase_date', 'created_at', 'stock__exchange']
    search_fields = ['user__email', 'stock__symbol', 'stock__name', 'notes']
    readonly_fields = ['id', 'total_cost', 'current_value', 'gain_loss', 'gain_loss_percent', 'created_at', 'updated_at']
    ...
    fieldsets = (
        ...
        ('Calculated Fields', {
            'fields': ('total_cost', 'current_value', 'gain_loss', 'gain_loss_percent'),
            'classes': ('collapse',)
        }),
        ('Metadata', {
            'fields': ('id', 'created_at', 'updated_at'),
...
class OrderAdmin(admin.ModelAdmin):
    list_display = [
        'user_email', 'stock_symbol', 'transaction_type', 'order_type',
        'quantity', 'status_display', 'target_price', 'executed_price',
        'executed_at', 'created_at'
    ]
    list_filter = [
        'transaction_type', 'order_type', 'status', 'created_at', 'executed_at', 'stock__exchange'
    ]
    search_fields = ['user__email', 'stock__symbol', 'stock__name', 'notes']
    ...
    fieldsets = (
        ...
        ('Order Details', {
            'fields': ('quantity', 'target_price', 'notes')
        }),
        ('Execution Details', {
            'fields': ('executed_price', 'executed_at'),
            'classes': ('collapse',)
        }),
        ('Calculated Fields', {
            'fields': ('can_execute',),
    ...
    def mark_as_cancelled(self, request, queryset):
        updated = queryset.filter(status__in=['waiting', 'insufficient_funds']).update(status='cancelled')
