...
from .models import Stock, StockPrice, UserWatchlist, StockAlert, StockTick, IntradayPrice, Portfolio, Order
...
class StockSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'market_cap', 'market_cap_formatted', 'description',
            'is_active', 'latest_price', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'market_cap_formatted', 'latest_price']
...
class StockPriceSerializer(serializers.ModelSerializer):
    ...
    price_change = serializers.DecimalField(max_digits=12, decimal_places=4, read_only=True)
    price_change_percent = serializers.DecimalField(max_digits=8, decimal_places=4, read_only=True)

    class Meta:
        model = StockPrice
        fields = [
            'id', 'stock', 'stock_symbol', 'stock_name', 'date', 'interval',
            'open_price', 'high_price', 'low_price', 'close_price', 'adjusted_close',
            'volume', 'price_change', 'price_change_percent', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'stock_symbol', 'stock_name', 'price_change',
            'price_change_percent', 'created_at', 'updated_at'
        ]
...
class StockPriceListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockPrice
        fields = [
            'id', 'stock_symbol', 'date', 'interval', 'open_price',
...
class UserWatchlistSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserWatchlist
        fields = [
            'id', 'stock', 'stock_symbol', 'stock_details', 'notes',
            'target_price', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'user', 'stock_symbol', 'stock_details', 'created_at', 'updated_at']
...
class StockAlertSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockAlert
        fields = [
            'id', 'stock', 'stock_symbol', 'stock_details', 'alert_type',
            'threshold_value', 'is_active', 'is_triggered', 'triggered_at',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'stock_symbol', 'stock_details', 'is_triggered',
...
class StockTimeSeriesSerializer(serializers.Serializer):
    ...
    interval = serializers.ChoiceField(
        choices=StockPrice.INTERVAL_CHOICES,
        default='1d'
    )
...
class StockTickSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockTick
        fields = [
            'id', 'stock', 'stock_symbol', 'price', 'volume',
            'bid_price', 'ask_price', 'bid_size', 'ask_size',
            'spread', 'spread_percentage', 'trade_type',
            'timestamp', 'is_market_hours', 'created_at'
        ]
        read_only_fields = [
            'id', 'stock_symbol', 'spread', 'spread_percentage', 'created_at'
        ]
...
class StockTickListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = StockTick
        fields = [
            'id', 'stock_symbol', 'price', 'volume',
            'bid_price', 'ask_price', 'spread', 'trade_type', 'timestamp'
        ]
...
class IntradayPriceSerializer(serializers.ModelSerializer):
    ...
    price_change = serializers.DecimalField(max_digits=12, decimal_places=4, read_only=True)
    price_change_percent = serializers.DecimalField(max_digits=8, decimal_places=4, read_only=True)
    typical_price = serializers.DecimalField(max_digits=12, decimal_places=4, read_only=True)

    class Meta:
        model = IntradayPrice
        fields = [
            'id', 'stock', 'stock_symbol', 'stock_name', 'timestamp', 'interval',
            'open_price', 'high_price', 'low_price', 'close_price', 'volume',
            'vwap', 'trade_count', 'session_type', 'price_change',
            'price_change_percent', 'typical_price', 'created_at'
        ]
        read_only_fields = [
            'id', 'stock_symbol', 'stock_name', 'price_change',
            'price_change_percent', 'typical_price', 'created_at'
...
class IntradayPriceListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = IntradayPrice
        fields = [
            'id', 'stock_symbol', 'timestamp', 'interval', 'open_price',
            'high_price', 'low_price', 'close_price', 'volume', 'vwap',
            'price_change', 'price_change_percent', 'session_type'
        ]
...
class RealTimeDataSerializer(serializers.Serializer):
    ...
    symbol = serializers.CharField(max_length=20)
    interval = serializers.ChoiceField(
        choices=IntradayPrice.INTRADAY_INTERVALS + [('5m', '5 Minutes'), ('15m', '15 Minutes'), ('30m', '30 Minutes')],
        default='1m'
    )
    ...
    def validate(self, data):
        ...
        if start_time and end_time and start_time > end_time:
            raise serializers.ValidationError(
                _("Start time cannot be after end time.")
            )
...
class PortfolioSerializer(serializers.ModelSerializer):
    ...
    current_value = serializers.DecimalField(max_digits=12, decimal_places=4, read_only=True)
    gain_loss = serializers.DecimalField(max_digits=12, decimal_places=4, read_only=True)
    gain_loss_percent = serializers.DecimalField(max_digits=8, decimal_places=4, read_only=True)
    current_price = serializers.SerializerMethodField()

    class Meta:
        model = Portfolio
        fields = [
            'id', 'stock', 'stock_symbol', 'stock_details', 'quantity',
            'purchase_price', 'purchase_date', 'notes', 'total_cost',
            'current_value', 'gain_loss', 'gain_loss_percent', 'current_price',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'stock_symbol', 'stock_details', 'total_cost',
            'current_value', 'gain_loss', 'gain_loss_percent', 'current_price',
            'created_at', 'updated_at'
        ]
...
class PortfolioCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Portfolio
        fields = ['stock_symbol', 'quantity', 'purchase_price', 'purchase_date', 'notes']
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'stock', 'stock_symbol', 'stock_details', 'transaction_type',
            'order_type', 'quantity', 'target_price', 'status', 'executed_price',
            'executed_at', 'notes', 'can_execute', 'current_price',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'user', 'stock_symbol', 'stock_details', 'status',
            'executed_price', 'executed_at', 'can_execute', 'current_price',
            'created_at', 'updated_at'
        ]
...
class OrderCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ['stock_symbol', 'transaction_type', 'order_type', 'quantity', 'target_price', 'notes']
    ...
    def validate_target_price(self, value):
        ...
        if value is not None and value <= 0:
            raise serializers.ValidationError(
                _("Target price must be greater than 0.")
            )
        return value
            ...
            if latest_price:
                ...
                try:
                    ...
                    if user_profile.cash < total_cost:
                        ...
                        order.save()
                        logger.warning(f"Order {order.id} set to insufficient_funds: need {total_cost}, have {user_profile.cash}")
                        return order
                except UserProfile.DoesNotExist:
                    order.status = 'insufficient_funds'
                    order.save()
                    logger.warning(f"Order {order.id} set to insufficient_funds: no user profile")
                    return order

        # Execute market orders immediately
        if order.order_type == 'market':
            logger.info(f"Executing market order for {order.stock.symbol} ({order.transaction_type}, {order.quantity} shares)")
            ...
            if not executed:
                ...
                if order.status == 'waiting':
                    ...
                    logger.warning(f"Execution failed for {order.stock.symbol} ({order.transaction_type}, {order.quantity} shares) - keeping in waiting status")
                elif order.status == 'insufficient_funds':
                    logger.warning(f"Order failed due to insufficient funds for {order.stock.symbol}")
            else:
